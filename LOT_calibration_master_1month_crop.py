...
size=1024
master_bias_cut=Cutout2D(master_bias,position,size).data
print('master bias cut: ', master_bias_cut.shape)
#plt.title('Master Bias cut')
#plt.imshow(master_bias_cut)
#plt.show()
#sys.exit(0)
print('------------')
...
hdu=fits.PrimaryHDU(data=master_bias_cut)
...
position=(1023.5,1023.5)
size=1024
master_dark_cut=Cutout2D(master_dark,position,size).data
print('master dark cut: ', master_dark_cut.shape)    
...
hdu=fits.PrimaryHDU(master_dark_cut)
...
#    cmd_search_file_flat='find ./ |grep '+dir_month+' | grep fts | grep flat | grep PANoRot-'+i
    cmd_search_file_flat='find ./ |grep '+dir_month+' | grep fits | grep flat |grep '+i
...
#    mean_flat=np.nanmean(flat_keep-master_bias-master_dark,axis=0)  
    ...
    flat_subtract=mean_flat-master_bias_cut-master_dark_cut
