...
dir_calib=yearmonth+'/'+folder+'_calib_'+exptime+'S/'
#dir_calib='data/'+yearmonth+'/'+folder+'_calib/'

print(dir_calib)
if os.path.exists(dir_calib):
    shutil.rmtree(dir_calib)
...
#    plt.show()
    ...
    fitsname_calib=sci_name+'_calib.fits'
    #hdu=fits.PrimaryHDU(calib_sci[i])
    fits.writeto(dir_calib+fitsname_calib,data=sci_flat,header=imhead,overwrite=True)
